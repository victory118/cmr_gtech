...
% see the LICENSE file included with this software
    ...
    %% METHODS
        ...
        %% SUPERVISOR Constructor
            ...
            obj.goal        = [1.1, 1.1];
            obj.d_stop      = 0.05;
            obj.d_at_obs    = 0.15; % 0.10               
            obj.d_unsafe    = 0.05; % 0.05
        ...
        %   See also controller/execute
            ...
            elseif ((obj.is_in_state('go_to_goal')||obj.is_in_state('ao_and_gtg')) && obj.check_event('at_obstacle'))
                if obj.sliding_left() && ~obj.progress_made()
                    obj.fw_direction = 'left';
                    obj.switch_to_state('follow_wall');
                elseif obj.sliding_right() && ~obj.progress_made()
            ...
            elseif (obj.is_in_state('follow_wall'))
                if obj.progress_made() && strcmp(obj.fw_direction, 'left') && ~obj.sliding_left()
                    obj.switch_to_state('go_to_goal');
                elseif obj.progress_made() && strcmp(obj.fw_direction, 'right') && ~obj.sliding_right()
                    obj.switch_to_state('go_to_goal');
                end
            else
                obj.switch_to_state('go_to_goal');
...
%                 rc = true;
%             end
            if norm([x-obj.goal(1);y-obj.goal(2)]) < obj.d_prog - 0.01
                rc = true;
            end
