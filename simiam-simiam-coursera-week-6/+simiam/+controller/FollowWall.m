...
%                 [~, ir_dist_idx] = sort(ir_distances(3:5));
%                 ir_dist_idx = ir_dist_idx + 2;
%                 p_1 = ir_distances_wf(:,max(ir_dist_idx(1:2)));
%                 if ir_distances(ir_dist_idx(2)) == ir_distances(ir_dist_idx(3))
%                     % if longer two distances are equal, choose the one
%                     % with the highest index
%                     p_2 = ir_distances_wf(:,max(ir_dist_idx(2:3)));
%                 else
%                     p_2 = ir_distances_wf(:,min(ir_dist_idx(1:2)));
%                 end
%                 p_1 = ir_distances_wf(:,max(ir_dist_idx(1:2)));
%                 p_2 = ir_distances_wf(:,min(ir_dist_idx(1:2)));
                [~, ir_dist_idx] = sort(flipud(ir_distances(3:5))); % reverse counting order before sorting
                ir_dist_idx = 6 - ir_dist_idx; % map reverse sorted index back to true index
                p_1 = ir_distances_wf(:, max(ir_dist_idx(1:2)));
                p_2 = ir_distances_wf(:, min(ir_dist_idx(1:2)));
            else
                % Pick two of the left sensors based on ir_distances
                [~, ir_dist_idx] = sort(ir_distances(1:3));
                p_1 = ir_distances_wf(:,min(ir_dist_idx(1:2)));
                p_2 = ir_distances_wf(:,max(ir_dist_idx(1:2)));
            end
            
%             u_fw_t = [0;0];
            u_fw_t = p_2 - p_1; % vector that estimates section of wall

            % 2. Compute u_a, u_p, and u_fw_tp to compute u_fw_p
            
%             u_fw_tp = [0;0];
            u_fw_tp = u_fw_t/norm(u_fw_t); % wall unit vector
%             u_a = [0;0];
            u_a = p_1; % global obstacle position
%             u_p = [0;0];
            u_p = [x; y]; % global robot position
            
%             u_fw_p = [0;0];
            u_fw_p = (u_a - u_p) - (dot(u_a - u_p, u_fw_tp)*u_fw_tp);% vector from robot to wall
            
            % 3. Combine u_fw_tp and u_fw_pp into u_fw;
%             u_fw_pp = [0;0];
            u_fw_pp = u_fw_p - d_fw*u_fw_p/norm(u_fw_p); % robot to wall unit vector
%             u_fw = u_fw_tp;
            c_wall = 0.3; % higher means it goes faster perpendicular
            % works for left wall following: c_wall = 0.3, d_fw = 0.15, 0.16
            % works for right wall following: c_wall = 0.3, d_fw =
            % 0.15, 0.16, 0.17
            u_fw = c_wall*u_fw_tp + (1-c_wall)*u_fw_pp;
